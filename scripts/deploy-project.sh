...
# Staging Pipeline — triggers on Stagging branch
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Upload Build Artifacts
        ...
        with:
          name: build-artifacts-Stagging-${{ github.sha }}
  ...
  deploy-staging:
    ...
    steps:
      ...
      - name: Deploy to Staging Gateway
        shell: powershell
        env:
          ...
          STAGING_GATEWAY_USER:    ${{ secrets.STAGGING_GATEWAY_USER }}
        run: |
          ...
          $env:STAGING_GATEWAY_USER    = "${{ secrets.STAGGING_GATEWAY_USER }}"

          # ✅ Smart path detection — works for combined AND standalone run
          $projectsPath = if (Test-Path ".\projects") { ".\projects" } else { ".\artifacts\projects" }
          $scriptBase   = if (Test-Path ".\projects") { "projects" }  else { "artifacts/projects" }
          Write-Host "Using projects path: $projectsPath"

          $projects = Get-ChildItem -Path $projectsPath -Directory
          if ($projects.Count -eq 0) {
            Write-Host "ERROR: No projects found in $projectsPath"
            exit 1
          }
          ...
          foreach ($project in $projects) {
            ...
            Write-Host "=========================================="
            Write-Host "Deploying $projectName to Staging"
            Write-Host "=========================================="
            $bashCmd = "STAGING_GATEWAY_URL='$env:STAGING_GATEWAY_URL' STAGING_GATEWAY_API_KEY='$env:STAGING_GATEWAY_API_KEY' STAGING_GATEWAY_PASS='$env:STAGING_GATEWAY_PASS' STAGING_GATEWAY_USER='$env:STAGING_GATEWAY_USER' ./scripts/deploy-project.sh staging $scriptBase/$projectName"
            & "C:\Program Files\Git\bin\bash.exe" -c $bashCmd
            Write-Host "✓ $projectName deployed to Staging"
          }
